...
| 기능                 | Figma 화면          | 상태         | 우선순위 |
| -------------------- | ------------------- | ------------ | -------- |
| 언어인지 불가 팝업   | 17/19.언어인지 불가 | ✅ 구현 완료 | 중간     |
| 대화종료 재확인 팝업 | 15.대화종료 팝업    | ✅ 구현 완료 | 중간     |
| 회원가입 권유 팝업   | 14.회원가입 권유    | ✅ 구현 완료 | 중간     |
| 실시간 힌트 UI       | 13.실시간 힌트      | ✅ 구현 완료 | 중간     |
...
### Phase 3: 언어인지 불가 팝업 (Figma 17/19) ✅ 완료

- [x] `features/chat/ui/LanguageNotRecognizedDialog.tsx` 생성
- [x] `[unintelligible]` 감지 시 팝업 트리거
- [x] 다시 말하기 / 텍스트 입력 전환 옵션 제공
- [ ] 텍스트 입력 모드 구현 (Phase 7로 연기)

### Phase 4: 대화종료 재확인 팝업 (Figma 15) ✅ 완료

- [x] `app/chat/layout.tsx`에서 ConfirmPopup으로 구현됨
- [x] 종료 버튼 클릭 시 확인 다이얼로그 표시
- [x] "계속하기" / "종료하기" 옵션

### Phase 5: 회원가입 권유 팝업 (Figma 14) ✅ 완료

- [x] `features/auth/ui/SignupPromptDialog.tsx` 생성
- [x] 게스트 사용자 대화 완료 시 트리거 (`app/chat/complete/page.tsx`)
- [x] 대화 기록 저장 혜택 안내 + 가입 유도

### Phase 6: 실시간 힌트 UI (Figma 13) ✅ 완료

- [x] `features/chat/ui/RealtimeHint.tsx` 구현 (힌트 버튼 + 말풍선)
- [x] REST API `useGetHints` 연동 (`/chat/hints/{sessionId}`)
- [x] 말풍선 스타일 (amber 그라데이션) + `animate-fade-in-up` 애니메이션
- [x] 테스트 12개 통과

### Phase 7: 테스트/품질 ✅ 완료

- [x] E2E 테스트 기본 구조 (143개 통과, 5개 스킵)
- [x] 단위 테스트 커버리지 확보 (57개 파일, 673개 테스트 통과)
- [x] 실패 테스트 수정 완료:
  - `debug.test.ts`: isDev() 항상 false 반환하도록 테스트 수정
  - `DebugStatus.test.tsx`: vi.mocked() 패턴으로 모킹 개선
  - `use-duplicate-check.test.ts`: 닉네임 유효성 규칙(2-6자) 준수하도록 테스트값 수정
...
| `v0.3.0` | Phase 3 (언어인지 불가 팝업)   | ✅ 완료 |
| `v0.4.0` | Phase 4 (대화종료 재확인 팝업) | ✅ 완료 |
| `v0.5.0` | Phase 5 (회원가입 권유 팝업)   | ✅ 완료 |
| `v0.6.0` | Phase 6 (실시간 힌트 UI)       | ✅ 완료 |
| `v0.7.0` | Phase 7 (테스트/품질)          | ✅ 완료 |
| `v1.0.0` | MVP 릴리즈                     | 예정    |
