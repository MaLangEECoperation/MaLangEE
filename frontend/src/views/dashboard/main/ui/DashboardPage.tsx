...
export interface DashboardPageProps {
  /** 페이지 텍스트 콘텐츠 (다국어 지원용) */
  contents?: DashboardPageContents;
}
...
export function DashboardPage({ contents = defaultDashboardContents }: DashboardPageProps) {
  ...
  const formatDuration = (seconds: number) => {
    ...
    return `${hours}${contents.timeFormat.hours} ${minutes}${contents.timeFormat.minutes} ${secs}${contents.timeFormat.seconds}`;
  };
  ...
  const leftContent = (
    <div className="w-full max-w-[90%] tracking-tight md:max-w-sm">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            aria-label={contents.profile.editNicknameLabel}
          >
            <Pencil size={16} stroke="currentColor" />
          </Button>
        </div>
        <Button variant="secondary" size="sm" onClick={handleLogoutClick}>
          {contents.profile.logoutButton}
        </Button>
      </div>
      <div className="mt-5 space-y-1">
        <div className="flex items-center justify-between">
          <span className="text-sm">{contents.profile.totalDurationLabel}</span>
          <span className="text-sm font-bold">
            {userProfile ? formatDuration(userProfile.totalDurationSec) : "0시간 0분 0초"}
          </span>
        </div>
        <div className="mt-2 flex items-center justify-between">
          <span className="text-sm">{contents.profile.userDurationLabel}</span>
          <span className="text-sm font-bold">
      ...
      <div className="mt-4 flex hidden justify-center">
        ...
        >
          {contents.profile.deleteAccountButton}
        </button>
      </div>
    </div>
  );
  ...
  const rightContent = (
    <div className="flex min-h-0 w-full flex-1 flex-col tracking-tight">
      ...
      <div className="mb-4 mt-0 flex w-full justify-start">
        <h2 className="text-2xl font-bold text-[#1F1C2B]">{contents.history.title}</h2>
      </div>
      {/* 대화 목록 */}
            ...
            <div className="left-0 flex w-full flex-col items-start justify-start pr-2 md:h-[400px] md:overflow-y-auto">
              ...
              {!hasNextPage && allSessions.length > 0 && (
                <div className="flex w-full items-center justify-center py-4 text-xs text-gray-500">
                  {contents.history.allLoadedMessage} (전체: {totalCount}건 / 조회:{" "}
                  {allSessions.length}건)
                </div>
              )}
            </div>
          </>
